...
class WordpieceIndexer(TokenIndexer[int]):
    ...
    def __init__(self,
        ...
        self._start_piece_ids = [vocab[token]
                                 for token in (start_tokens or [])]

        self._end_piece_ids = [vocab[token]
                               for token in (end_tokens or [])]

        # Convert the separator_token to wordpiece_ids
        self._separator_ids = [vocab[separator_token]]
    ...
    def tokens_to_indices(self,
        ...
        for token in tokens:
            ...
            if len(all_token_ids) + len(token_ids)  <= self.max_pieces:
        ...
        token_type_ids = _get_token_type_ids(all_token_ids,
                                             self._separator_ids)
        ...
        mask = [1 for _ in all_token_ids]

        return {
                index_name: all_token_ids,
    ...
    def get_keys(self, index_name: str) -> List[str]:
        ...
        """
        # pylint: disable=no-self-use
        return [index_name, f"{index_name}-type-ids", "mask"]


@TokenIndexer.register("bert-pretrained2")
